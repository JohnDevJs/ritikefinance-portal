...
function LoanForm({ onClose, reFetch }) {
    ...
    const onChangeDate = ({ target }) => {
        ...
        */
        ...
        console.log("Payment date: ", newDate);

        // Get the current date
        const today = new Date();
        today.setHours(0, 0, 0, 0);  // Set hours, minutes, seconds, and milliseconds to 0 for accurate calculations

        // Calculate the difference in milliseconds
        const diffInMs = newDate - today;

        // Convert milliseconds into days (1 day = 24*60*60*1000 = 86400000 milliseconds)
        const diffInDays = diffInMs / 86400000;

        console.log("Days until payment: ", diffInDays);
