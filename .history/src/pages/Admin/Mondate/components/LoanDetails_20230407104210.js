...
function LoanDetails({ loan_Id }) {
    ...
    const { data, loading, error, reFetch } = useFetch(
        `${process.env.REACT_APP_BACKEND_URL}/mandates/${loan_Id}`,
        token
    );
    const TotalToPay = parseInt(data?.loan?.totalAmount) + parseInt(data?.loan?.amount);
    ...
    return (
        <Row>
            <div className="mt-4">
                <p>
                    <b> Full Name : </b> {data?.user?.firstName} {data?.user?.lastName}
                </p>
                <p>
                    <b>Phone number : </b>
                ...
                <p>
                    <b>Payment date : </b>
                    {data?.loan?.paymentDate.split('T')[0]}
                </p>
                <p>
                    <b>Loan amount : </b>R {data?.loan?.amount}
                </p>
                <p>
                    <b>Amount to pay : </b>R {TotalToPay}
                </p>
            </div>
        </Row>
    );
...
function LoanDetailsInModal({ loan_Id, isOpen, onClose }) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            doc.open();
            doc.write(`<html><head><style>body{margin:0;padding:0;}</style></head><body>${iframe.contentWindow.document.body.innerHTML}</body></html>`);
            doc.close();
        }
    }, [isOpen]);
    ...
    return (
        <div>
            <iframe
                title="Loan Details"
                ref={iframeRef}
                style={{ display: isOpen ? 'block' : 'none', border: 'none', width: '100%', height: '100%' }}
                srcDoc={`<!DOCTYPE html><html><head><style>body{margin:0;padding:0;}</style></head><body><div style="height: 100%;"><LoanDetails loan_Id={loan_Id} /></div></body></html>`}
            />
        </div>
    );
}

export default LoanDetailsInModal;
