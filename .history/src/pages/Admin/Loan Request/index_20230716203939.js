...
import DeclinedLoan from "./components/DeclinedLoan";
...
const Index = () => {
    ...
    const res_data = [];
    const { data, loading, error, reFetch } = useFetch(`${process.env.REACT_APP_BACKEND_URL}/loans?status=pending&status=verification&sort=-createdAt`, token);
    ...
    return (
        <React.Fragment>
            ...
            <SmallModal
                ...
                ModalTitle={modalTitle}
                cancel="close"
                Components={<DeclinedLoan reFetch={reFetch} onClose={() => setDeclineModal(false)} loanId={loanId} btnName={btnName} />}
            />
