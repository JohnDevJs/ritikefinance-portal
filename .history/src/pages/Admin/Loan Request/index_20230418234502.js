...
import { ApprovedLoanMsg, DeclinedLoanMsg, SendFormMsg, ServerError, VerificationLoanMsg } from "components/NotifyMessage";
    ...
    const filterArr = () => {
        data.forEach(res => {
            res_data.push({
                ...
                downloadBtn: <button className="btn btn__table color__download" onClick={() => sendFormFunc(res?.user?._id, res?._id)}> Send form  </button>,
                ...
                formBtn: <button className="btn btn__table color__red" onClick={() => declineFunc(res?._id)}> Declined </button>,
