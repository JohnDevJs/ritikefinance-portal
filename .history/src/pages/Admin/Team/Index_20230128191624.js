...
const Index = () => {
    ...
    const filterArr = () => {
        data.forEach(res => {
            res_data.push({
                ...res,
                actionBtn: <button onClick={() => suspendUser(res._id)} className={`btn color__red`}> Suspend </button>,
                actionBtn: <button onClick={() => unSuspend(res._id)} className={`btn color__black`}> UnSuspend </button>,
