...
const Index = () => {
    ...
    const { data, loading, length, error } = useFetch(`${process.env.REACT_APP_BACKEND_URL}/loans`, null);
    ...
    const filterArr = () => {
        data.forEach(res => {
            res_data.push({
                ...
                progressBar: <div className="text-center">% {res.loanPercentage} <Progress color={res?.loanPercentage <= 50 ? "danger" : res?.loanPercentage <= 99 ? "info" : "success"}
                    value={data?.loanPercentage} /> </div>,
