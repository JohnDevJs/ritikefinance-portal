...
function SuspendComp({ reFetch, onClose, user_Id, btnName, apiQuery }) {
    ...
    const changeStatusFunc = () => {
        ...
        const Method = apiQuery === "Deleted" ? 'DELETE' : 'POST';
        const endPoint = apiQuery === "Deleted" ? `users/${user_Id}` : (apiQuery === "Generated" ? `monthlyTotals/${user_Id}/monthly-totals` : `users/${user_Id}/${apiQuery}`);
