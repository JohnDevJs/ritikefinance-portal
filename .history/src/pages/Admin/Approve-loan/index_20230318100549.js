...
const Index = () => {
    ...
    const res_data = [];
    const { data, loading, error, reFetch } = useFetch(`${process.env.REACT_APP_BACKEND_URL}/loans?status=approve`, token);
    ...
    const filterArr = () => {
        data.forEach(res => {
            res_data.push({
                ...
                verifyBtn: <button className="btn btn__table color__verify" onClick={() => updatePayment(res?._id)}>  Update </button>,
    ...
    return (
        <React.Fragment>
            ...
            <SmallModal
                ...
                ModalTitle="Update payment loan"
                cancel="close"
